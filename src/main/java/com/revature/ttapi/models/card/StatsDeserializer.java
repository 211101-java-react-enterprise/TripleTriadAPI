...
public class StatsDeserializer extends StdDeserializer<Stats> {
    ...
    public Stats deserialize(JsonParser jp, DeserializationContext deserializationContext) throws IOException {
        ...
        JsonNode statsNode = jp.getCodec().readTree(jp);
        Stats s = new Stats();
        s.setBottom(statsNode.get("numeric").get("bottom").asInt());
        s.setTop(statsNode.get("numeric").get("top").asInt());
        s.setLeft(statsNode.get("numeric").get("left").asInt());
        s.setRight(statsNode.get("numeric").get("right").asInt());
        return s;
    }
}
